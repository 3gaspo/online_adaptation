...
For the deadline experiment, four DGX fronts narrow this grid. The fixed
...
query stride 127. Here $\Nfit$ is an integer count: it retains every complete
stride-24 fitting date in T1+T2, namely 548 dates for Electricity and 182 for
Solar. The full $2\times2\times2$ comparison crosses fixed versus rolling
...
shares the exact T3 query-date grid and a global maximum of 20,000 datastore
...
first anchors T3 and 30 fitting dates at stride 24, then fills the preceding
datastore up to 20,000 windows. Its datastore is periodically aligned with
...
fixed, unstrided, same-variable T0 datastore, the same global 20,000-window
maximum, and $K=5$.
