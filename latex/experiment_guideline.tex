...
\newcommand{\Nstore}{N_{\mathrm{store}}}
...
\]
The task is to predict every $y_{q,u}$. There is no train/validation/test
split. At date $q$, a row dated $r$ may be retrieved or fitted only if
\[
 r+H\le q,                                                     \tag{1}
\]
...
Let $\Nstore$ be the datastore-size setting and let $M$ be its maximum number
of dates:
\[
 M=
 \begin{cases}
 \Nstore, & \text{same-user retrieval},\\
 \lfloor\Nstore/U\rfloor, & \text{all-user or other-user retrieval}.
 \end{cases}                                                   \tag{4}
\]
Thus cross-user retrieval requires $\Nstore\ge U$ and never keeps a partial
oldest date. A rolling store $\mathcal D_q$ contains the latest $M$ dates of
$\mathcal D_q^\infty$; a fixed store contains its earliest $M$ dates.
...
\[
 ...
 \end{cases}
\]
where $P\mid\Sfit$ means that $\Sfit$ is a multiple of $P$. The rolling
fitting set is the latest $\Nfit$ dates in
\[
 \mathcal F_q^\infty
 =\{c_q-j\Sfit:j\in\mathbb N_0,\ c_q-j\Sfit\ge L-1\}.        \tag{5}
\]
...
A fixed fit freezes the first complete $\mathcal F_q$ for each phase modulo
$\Sfit$.

The first evaluated date $q_0$ is the first date for which $q_0$ and every
date in $\mathcal F_{q_0}$ have $\Kmax$ retrievable neighbors and
$|\mathcal F_{q_0}|=\Nfit$. The evaluated dates are
\[
...
raises an error; it is never replaced by $V_{q,u}$. The five-backbone profile
...
The current gate
front uses $A=C$, $K=20$, and backbones with (10); hence its predictions are
exactly $V$ under the current code.
...
Chronos-Bolt, and same-user retrieval. Its retriever uses the final Chronos-T5
...
lookback--future sequences. Only the set of dates allowed by (1)--(4) differs
from the released retrieval procedure.
...
Methods are compared only on their common query dates. If $m$ denotes an
...
rolling store and fit, $\Nstore=30{,}000$, $\Nfit=100$,
$\Sstore=\Sfit=P$, seed $1$, and (14). Test runs use $\Sq=257$; small and full
runs use $\Sq=127$. The frozen backbone is Chronos-2. The main family also adds
the TS-RAG configuration defined above.
...
Front & Values \\
\midrule
\path{ablation_n_store.slurm} & test: $\{1000,2000,5000\}$; publication:
$\{10000,20000,30000,50000\}$ \\
\path{ablation_n_fit.slurm} & test: $\{50,100,500\}$; publication adds $1000$ \\
...
\path{ablation_fixed_protocol.slurm} & rolling/fixed store crossed with rolling/fixed fit \\
...
\path{ablation_backbones.slurm} & Chronos-2, Chronos-Bolt, TS-ICL, and TabPFN-TS at $(512,64)$; TiRex-2 remains implemented but excluded \\
...
The gate front evaluates \texttt{bayes\_cov\_shared\_soft} and
\texttt{catboost\_cov\_shared\_soft}. CatBoost uses 300 trees in small/full
and 2 trees in test, depth $4$, learning rate $0.03$, and refit stride $1$.
...
\footnotesize
\begin{algorithmic}[1]
\Require panel $Z$; settings $L,H,\Nstore,\Nfit,\Sstore,\Sfit,\Sq,\Kmax$
...
\ForAll{$q\in\mathcal Q$}
  \State build $\mathcal F_q$ from (5)
\EndFor
...
\ForAll{$r\in\mathcal F_{q_0}\cup\{q_0\}$ first, then remaining $r\in\mathcal R$ by date}
  \State build $\mathcal D_r$ from (3)--(4)
  \For{$u=1$ to $U$}
...
\end{center}
Date selection is in \path{src/proposal/datastore.py}; ridge designs and solves
...
Each root \texttt{.slurm} file sets one family and calls
...
Generated arrays, metrics, figures, and tables are under \path{outputs/}; run
logs are under \path{logs/}.
