...
non-fixed datastore and training set, $M=100$, $\Nfit=100$,
...
For the deadline experiment, two temporary jobs narrow this grid.
The fixed ablation uses Electricity and Solar at only the short and long
settings. It first fixes T0/T1+T2/T3 to 30/50/20 percent of each source
timeline, uses periodically aligned datastore origins, fit stride 24, and T3
query stride 127, and crosses
fixed versus rolling datastore with fixed versus online ridge. The four arms
share the exact T3 query-date grid and the same global 20,000-window maximum;
ridge selection uses the chronologically newest 20 percent of the available
T1+T2 fitting dates for validation.

The temporary TIME comparison fixes $(L,H)=(512,64)$ and hard-codes
\path{ne_china_wind_h}, \path{coastal_t_s_h_part11}, and
\path{sg_weather_d}. Both methods evaluate the same stride-127 T3 grid beginning
at the 80-percent boundary. Full online ridge uses a rolling all-user datastore
capped at 20,000 windows, $\Nfit=30$, $\Sfit=24$, and selects $K$ from
$\{1,5,10,15\}$ together with alpha. TS-RAG uses the same global 20,000-window
maximum, drawn from its fixed, unstrided, same-variable T0 datastore, and
$K=5$.
